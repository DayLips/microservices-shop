    ...
    try:
        with transaction.atomic():
            ...
            if not ProductService.reserve_products(items_to_reserve):
                return Response({
                    'error': 'Failed to reserve products. Some items may be out of stack.'
                }, status=status.HTTP_400_BAD_REQUEST)
...
def update_order_status(request, pk):
    ...
    if serializer.is_valid():
        ...
        if not is_valid_status_transaction(old_status, new_status):
            return Response({
...
def is_valid_status_transaction(old_status: str, new_status: str) -> bool:
    valid_transitions = {
