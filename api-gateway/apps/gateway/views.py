...
class ProxyView(View):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        service_name = self.get_sevice_name(request)
        if not service_name:
            logger.error(f"Service not found for path: {request.path}")
            return JsonResponse({'error': f"Service not found"}, status=404)
        ...
        return self.proxy_requset(request, target_url)
    
    def get_sevice_name(self, request):
        path = request.path

        if path.startwith('/api/products/') or path.startwith('/api/categories/'):
            return 'product-service'
        elif path.startwith('/api/auth/') or path.startwith('/api/users/'):
            return 'user-service'
        elif path.startwith('/api/cart/'):
            return 'cart-service'
        elif path.startwith('/api/order/'):
            return 'order-service'
        
        return None
    
    def proxy_requset(self, request, target_url):
        try:
            headers = {}
            important_headers = {
